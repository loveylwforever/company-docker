...
| `company-auth/fail-insert/` | auth 落库失败本地 SQL |
| `company-auth/logs/` | auth 滚动日志（`/var/log/company-auth`，主文件 `company-auth.log`） |
| `company-auth-channel/plugins/` | 渠道插件 JAR（发布升级写入） |
| `company-auth-channel/logs/` | auth-channel 滚动日志（`/var/log/company-auth-channel`，主文件 `company-auth-channel.log`） |
| `company-manage/logs/` | manage 滚动日志（`/var/log/company-manage`） |
...
`auth` / `auth-channel` / `manage` 默认 `SPRING_PROFILES_ACTIVE=prod`，业务日志同时写 stdout 与 `./company-auth/logs` 等目录（格式一致）；`docker compose logs` 与文件内容对齐。
...
渠道发布 JAR 落盘：`./company-auth-channel/plugins`（容器内 `/data/plugins`）。
...
```bash
sudo chown -R 100:101 ./company-manage/logs
sudo chown -R 1001:1001 ./activemq-artemis/artemis-instance
```
...
```bash
tail -50 company-auth/logs/company-auth-error.log
```
